...
class IntoCSV:
    ...
    def __init__(self, ):
        ...
        faces.sort()
        for person in faces:
            self.write_into_csv(self.path_photos_from_camera + person, self.path_csv_from_photos + person.split(".")[0] + ".csv")
        # 存放所有特征均值的 CSV 的路径
    ...
    def write_into_csv(self, path_faces_personX, path_csv_from_photos):
        ...
        with open(path_csv_from_photos, "w", newline="") as csvfile:
            ...
            features_128d = self.return_128d_features(path_faces_personX)
            # 遇到没有检测出人脸的图片跳过
